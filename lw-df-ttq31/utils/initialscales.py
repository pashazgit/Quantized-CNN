...
def initial_scales(opt_fp, thr):
    ...
    for group_fp in opt_fp.param_groups:
        ...
        # w_p = torch.ones((params_fp.size(0), 1), requires_grad=True)
        # assert (w_p.is_leaf)
        # if torch.cuda.is_available():
        #     w_p = w_p.cuda().detach().requires_grad_(True)
        # assert (w_p.is_leaf)
        # sf_g.append(w_p)
        # w_n = -torch.ones((params_fp.size(0), 1), requires_grad=True)
        # # assert (w_n.is_leaf)
        # if torch.cuda.is_available():
        #     w_n = w_n.cuda().detach().requires_grad_(True)
        # assert (w_n.is_leaf)
        # sf_g.append(w_n)
