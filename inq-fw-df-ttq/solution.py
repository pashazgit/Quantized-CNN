...
import sys
sys.path.insert(0, "./inq-fw-df-ttq/input_pipeline")
sys.path.insert(0, "./inq-fw-df-ttq/inq")
sys.path.insert(0, "./inq-fw-df-ttq/utils")
import os
import numpy as np
import torch
...
def main(config):
    ...
    if config.mode == "train":
        ...
        model_file_path = os.path.join(config.save_dir, "model.pth")
        ...
        res = torch.load(os.path.join(config.model_pretrained, 'resnet101-pretrained.pth'))
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=list(map(float, config.milestones.strip('[]').split(','))))
        ...
        for rnd in tqdm(range(len(steps)), desc=prefix):
            ...
            scheduler_fp = optim.lr_scheduler.MultiStepLR(optimizer_fp, milestones=[7])
            if torch.cuda.is_available():
            ...
            optimizer_sf = optim.Adam(sf, lr=config.learning_rate)
            scheduler_sf = optim.lr_scheduler.MultiStepLR(optimizer_sf, milestones=[7])
            if torch.cuda.is_available():
            ...
            train_writer = SummaryWriter(
                log_dir=os.path.join(config.log_dir, "train_{}".format(rnd)))
            val_writer = SummaryWriter(
                log_dir=os.path.join(config.log_dir, "valid_{}".format(rnd)))
