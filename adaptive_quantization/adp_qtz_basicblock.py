...
def train(config):
    ...
    train_writer = SummaryWriter(
        log_dir=os.path.join(config.log_dir, "train_adp_qtz_basicblock"))
    val_writer = SummaryWriter(
        log_dir=os.path.join(config.log_dir, "valid_adp_qtz_basicblock"))
    ...
    for epoch in tqdm(range(start_epoch, config.num_epoch)):
        # print("adp_qtz_basicblock_epoch: ", epoch)
        if epoch == 30:
...
class MyConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # s_c = t ** self.gamma / \
        #     torch.sum(t ** self.gamma, dim=-1, keepdim=True)  # the secondary coefficients
        if self.mode == 'train':
...
def model_loss(config, model):
    ...
    for name, param in model.named_parameters():
        ...
        if 'q_level' in name:
            ...
            # s_c = t ** config.gamma / \
            #       torch.sum(t ** config.gamma, dim=-1, keepdim=True)  # the secondary coefficients
            conv_w = torch.matmul(s_c, q_level)  # the conv layer's weights
            loss += torch.sum(conv_w ** 2)
...
def display_entropy(model, iter_idx):
    e = []  # entropy
    for name, param in model.named_parameters():
        if 'p_c' in name:
            p_c = param
