...
import sys
sys.path.insert(0, "./lw-df-ttq31/input_pipeline")
sys.path.insert(0, "./lw-df-ttq31/utils")
import os
import numpy as np
import torch
...
import torchvision
from utils.initialscales import initial_scales
...
def main(config):
    ...
    if config.mode == "train":
        ...
        model_file_path = os.path.join(config.save_dir, "model_lw_df_ttq31.pth")
        ...
        train_writer = SummaryWriter(
            log_dir=os.path.join(config.log_dir, "train_lw_df_ttq31"))
        val_writer = SummaryWriter(
            log_dir=os.path.join(config.log_dir, "valid_lw_df_ttq31"))
