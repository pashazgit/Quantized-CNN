...
import sys
sys.path.insert(0, "./fw-sf-tq3/input_pipeline")
sys.path.insert(0, "./fw-sf-tq3/utils")
import os
import numpy as np
import torch
...
def train_model(config, model, criterion, optimizer, optimizer_fp,
                train_loader, val_loader, train_writer, val_writer, iter_idx,
                scheduler, scheduler_fp, model_file_path):
    best_val_acc1 = 0
    ...
    for epoch in tqdm(range(config.num_epoch), desc=prefix):
        ...
        for x, y in train_loader:
            ...
            get_grads(optimizer, optimizer_fp)
            optimizer.step()
            optimizer_fp.step()

            ternarize(optimizer, optimizer_fp)
...
def main(config):
    ...
    if config.mode == "train":
        ...
        model_file_path = os.path.join(config.save_dir, "model_fw_sf_tq3.pth")
        ...
        train_writer = SummaryWriter(
            log_dir=os.path.join(config.log_dir, "train_fw_sf_tq3"))
        val_writer = SummaryWriter(
            log_dir=os.path.join(config.log_dir, "valid_fw_sf_tq3"))

        iter_idx = -1  # make counter start at zero

        train_model(config, model, criterion, optimizer, optimizer_fp,
                    train_loader, val_loader, train_writer, val_writer, iter_idx,
                    scheduler, scheduler_fp, model_file_path)
