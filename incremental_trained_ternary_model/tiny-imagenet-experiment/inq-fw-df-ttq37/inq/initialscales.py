...
def initial_scales(opt_fp, thr):
    ...
    for group_fp in opt_fp.param_groups:
        ...
        # a_p = (params_fp > delta).float() * params_fp * group_fp['part']
        # b_p = (params_fp > delta).float() * group_fp['part']
        # w_p = a_p.sum(dim=-1, keepdim=True)/b_p.sum(dim=-1, keepdim=True)
        a_p = (params_fp > delta).float() * params_fp * group_fp['part']
        w_p = a_p.sum(dim=-1, keepdim=True)/group_fp['part'].sum(dim=-1, keepdim=True)
        w_p.requires_grad = True
        if torch.cuda.is_available():
            w_p = w_p.cuda()
        sf_g.append(w_p)
        # a_n = (params_fp < -delta).float() * params_fp * group_fp['part']
        # b_n = (params_fp < -delta).float() * group_fp['part']
        # w_n = a_n.sum(dim=-1, keepdim=True)/b_n.sum(dim=-1, keepdim=True)
        a_n = (params_fp < -delta).float() * params_fp * group_fp['part']
        w_n = a_n.sum(dim=-1, keepdim=True)/group_fp['part'].sum(dim=-1, keepdim=True)
